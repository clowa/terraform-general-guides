...
*The master branch of the _live_ repository should be a 1:1 representation of what's actually deployed in production._
...
4. Create a _modules_ folder and implement the API you came up with as a collection of small, reusable, composable modules. Use a combination of Terraform and other tools like Docker, Packer, and Bash to implement these modules. Make sure to pin your Terraform and provider versions.
...
|Self-managed distributed system (stateful)  |Amazon ES                                          |2-4 months     |
...
|-|----------------|-------------------|
| Use version control | <ul><li>`git clone`<li>one repo per app<li>use branches</ul> | <ul><li>`git clone`<li>_live_ and _modules_ repos<li>_don't_ user branches</ul> |
...
| Submit changes for review | <ul><li>Submit a pull request<li>enforce coding guidelines</ul> | <ul><li>Submit a pull request<li>Enforce coding guidelines</ul> |
...
Do not use comments to explain what the code does, the code should do that itself. Only use comments to offer information that can't be expressed in code, such as _how the code is meant to be used_ or _why_ the code uses a particular design choice.
